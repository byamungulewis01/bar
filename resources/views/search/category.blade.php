                  ...
                  <div class="d-flex align-items-center mb-3">
                    <a href="{{ route('result', $user->id) }}" class="d-flex align-items-center">
                      <div class="avatar me-2">
                        <img src="{{ asset('assets/img/avatars/')}}/{{$user->user->photo}}" alt="Avatar" class="rounded">
                      </div>
                      <div class="me-2 text-body ms-1">
                        <h5 class="mb-0">{{ $user->user->name }}</h5>
                        <div class="client-info"><strong>Status: </strong><span class="badge bg-{{ badge($user->user->practicing) }}">{{ userStatus($user->user->practicing) }}</span></div>
                      </div>
                    </a>
                  </div>
                  <a href="{{ route('result', $user->user->id) }}" class="mb-3 text-body">
                    based in {{ $user->user->district }} distict, {{ $user->user->name }} have roll number of {{ $user->user->regNumber }} and was admitted on {{ $user->user->date->format('d/m/Y') }}  click to view more
